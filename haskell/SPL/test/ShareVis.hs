...
mapMemRaw :: (Show a) => [a] -> MemoryTable
mapMemRaw [] = [(getName [], ("null", []))]
mapMemRaw (a:as) = (getName a, (show a, [getName as])) : mapMemRaw as

graphRaw d = ([(loc, label) | (loc, (label, _)) <- memoryTable],
           concat [[(loc, n, "next") | n <- next] | (loc, (label, next)) <- memoryTable])
         where memoryTable = mapMemRaw d

graphRawFromTable table = ([(loc, label) | (loc, (label, _)) <- table],
           concat [[(loc, n, "next") | n <- next] | (loc, (label, next)) <- table])

showGraphRaw table = do
    let (vs, es) = graphRawFromTable table
        tempFileName = "temp"
